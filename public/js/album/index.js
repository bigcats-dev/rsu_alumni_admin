...
(function ($) {
    $(document).ready(function () {
        ...
        if (sessionStorage.hasOwnProperty('tab_album')
            && sessionStorage.tab_album != 'undefined'
            && $(`ul#tab-status li a[data-type="${sessionStorage.tab_album}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_album}"]`).tab('show')
        } else {
            // first tab
            $("ul#tab-status li:first-child a").tab('show')
        }
        // save active tab to session storage
        sessionStorage.setItem('tab_album', $("ul#tab-status li a.active").attr('data-type'))
        ...
        $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
            // save active tab to session storage
            sessionStorage.setItem('tab_album', $(event.target).attr('data-type'))
