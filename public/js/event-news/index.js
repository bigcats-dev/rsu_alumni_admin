...
(function ($) {
    $(document).ready(function () {
        ...
        if (sessionStorage.hasOwnProperty('tab_event_news')
            && sessionStorage.tab_event_news != 'undefined'
            && $(`ul#tab-status li a[data-type="${sessionStorage.tab_event_news}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_event_news}"]`).tab('show')
        } else {
            // first tab
            $("ul#tab-status li:first-child a").tab('show')
        }
        // save active tab to session storage
        sessionStorage.setItem('tab_event_news', $("ul#tab-status li a.active").attr('data-type'))
        ...
        $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
            // save active tab to session storage
            sessionStorage.setItem('tab_event_news', $(event.target).attr('data-type'))
