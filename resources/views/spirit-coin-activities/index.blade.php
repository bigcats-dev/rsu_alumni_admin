                ...
                <div class="col-sm-6">
                    <h1 class="m-0">
                        ...
                        @can("create-spirit-coin-activity")
                            <a href="{{route("spirit-coin-activity.create")}}" class="btn  btn-success"><i class="far fa-plus"></i>
                                สร้างกิจกรรมสปิริตคอยน์
                            </a>
                        @endcan
                    </h1>
                </div>
    ...
    <div class="modal fade" id="modalDisApprove" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <form action="{{route("spirit-coin-activity.approve",["coin" => ":id"])}}" id="frmDisApprove" method="POST">
                    @csrf
                    <input type="hidden" name="action" value="2">
                    <div class="modal-header">
                        <h4 class="modal-title"><strong>ไม่อนุมัติ กิจกรรมสปิริตคอยน์ <i class="far fa-times-circle text-danger"></i> </strong></h4>
...
@section("script")
    <script>
        var configTable = Object.assign($.fn.dataTable.defaults,{
            ...
            order:[[ 0, "asc" ]],
            ajax: {
                url: '{{ route("spirit-coin-activity.index") }}',
                type: "GET",
                data: function(data) {
                    data.type = sessionStorage.hasOwnProperty('tab_spirit_coin_activity')
                            ? sessionStorage.tab_spirit_coin_activity
                    ...
                    render: function(value, type, row){
                        if (row.status == '1') {
                            switch (value) {
                                case '0':
                                    return(`
                                        @can("approve-spirit-coin-activity")
                                            <form id="frmApprove_${row.spirit_coin_activity_id}" method="POST" action="${row.action.approve}" class="mb-2">
                                                @csrf
                                            ...
                                            <button type="button" class="btn btn-danger btn-block btn-sm btn-flat" data-id="${row.spirit_coin_activity_id}" data-toggle="modal" data-target="#modalDisApprove">
                                    ...
                                    return(`
                                        <div class="row">
                                            <div class="col">
                                                ...
                                                @can("del-spirit-coin-activity")
                                                    <form id="frmDestroy_${row.spirit_coin_activity_id}" method="POST" action="${row.action?.delete}">
                                                        @csrf
                            ...
                            return(`
                                <form method="POST" action="${row.action?.restore}" id="frmReStore_${row.spirit_coin_activity_id}">
                                    @csrf
                                    <button type="button" class="btn btn-primary btn-block btn-sm btn-flat" id="btn-restore_${row.spirit_coin_activity_id}">
            ...
            rowCallback: function(row, data) {
                const {spirit_coin_activity_id,action} = data
                $(row).find('[data-toggle="tooltip"]').tooltip()
                // approve
                $(row).find(`form[id="frmApprove_${spirit_coin_activity_id}"] button[type="button"]`)
                ...
                // delete
                $(row).find(`form[id="frmDestroy_${spirit_coin_activity_id}"] button[type="button"]`)
                    .on('click',async function(e){
                        e.preventDefault()
                        const rs = await confirmAlert.fire({
                ...
                // restore
                $(row).find(`button[id="btn-restore_${spirit_coin_activity_id}"]`)
                    .on('click',async function(e){
                        e.preventDefault()
                        const rs = await confirmAlert.fire()
                        if(rs) {
    ...
    <script src="{{ URL::asset("js/spirit-coin-activity/index.js?t=".time()) }}"></script>
@endsection