...
(function ($) {
    $(document).ready(function () {
        ...
        if (sessionStorage.hasOwnProperty('tab_spirit_coin')
            && sessionStorage.tab_spirit_coin != 'undefined'
            && $(`ul#tab-status li a[data-type="${sessionStorage.tab_spirit_coin}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_spirit_coin}"]`).tab('show')
        } else {
            // first tab
            $("ul#tab-status li:first-child a").tab('show')
        }
        // save active tab to session storage
        sessionStorage.setItem('tab_spirit_coin', $("ul#tab-status li a.active").attr('data-type'))
        ...
        $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
            // save active tab to session storage
            sessionStorage.setItem('tab_spirit_coin', $(event.target).attr('data-type'))
