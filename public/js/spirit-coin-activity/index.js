...
(function ($) {
    $(document).ready(function () {
        ...
        if (sessionStorage.hasOwnProperty('tab_spirit_coin_activity')
            && sessionStorage.tab_spirit_coin_activity != 'undefined'
            && $(`ul#tab-status li a[data-type="${sessionStorage.tab_spirit_coin_activity}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_spirit_coin_activity}"]`).tab('show')
        } else {
            // first tab
            $("ul#tab-status li:first-child a").tab('show')
        }
        // save active tab to session storage
        sessionStorage.setItem('tab_spirit_coin_activity', $("ul#tab-status li a.active").attr('data-type'))
        ...
        $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
            // save active tab to session storage
            sessionStorage.setItem('tab_spirit_coin_activity', $(event.target).attr('data-type'))
