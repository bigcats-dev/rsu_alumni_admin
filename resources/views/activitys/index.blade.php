...
@section('content')
    ...
    <div class="modal fade" id="modalAprove" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <form id="frmApprove" action="{{route("event-news.approve",["activity" => ":id"])}}" method="POST">
                    @csrf
                    <input type="hidden" name="action" value="1">
                    <div class="modal-header">
                        <h4 class="modal-title"><strong>ยืนยันอนุมัติ ข่าวสารกิจกรรม <i class="fas fa-check-circle text-success"></i> </strong></h4>
    ...
    <div class="modal fade" id="modalDisApprove" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <form action="{{route("event-news.approve",["activity" => ":id"])}}" id="frmDisApprove" method="POST">
                    @csrf
                    <input type="hidden" name="action" value="2">
                    <div class="modal-header">
                        <h4 class="modal-title"><strong>ไม่อนุมัติ ข่าวสารกิจกรรม <i class="far fa-times-circle text-danger"></i> </strong></h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div> 
                    <div class="modal-body">
                        <div class="col">
...
@section("script")
    <script>
        var configTable = Object.assign($.fn.dataTable.defaults,{
            ...
            order:[[ 0, "asc" ]],
            ajax: {
                url: '{{ route("event-news.index") }}',
                type: "GET",
                data: function(data) {
                    data.type = sessionStorage.hasOwnProperty('tab_event_news')
                            ? sessionStorage.tab_event_news
            ...
            rowCallback: function(row, data) {
                const {activity_id,action} = data
                $(row).find('[data-toggle="tooltip"]').tooltip()
                // priority
                $(row).find(`select[id="sl-priority_${activity_id}"]`)
                    .on('change',function(e){
                        $.post(action.priority,{priority: $(this).val()},function(){},'json')
                    })
                // show home page
                $(row).find(`input[id="show_homepage_${activity_id}"]`)
                    .on('click',function(e){
                        $.post(action.showhomepage,function(){},'json')
                    })
                // delete
                $(row).find(`form[id="frmDestroy_${activity_id}"] button[type="button"]`)
                ...
                // restore
                $(row).find(`button[id="btn-restore_${activity_id}"]`)
                    .on('click',async function(e){
                        e.preventDefault()
                        const rs = await confirmAlert.fire()
                        if(rs) {
    ...
    <script src="{{ URL::asset("js/event-news/index.js?t=".time()) }}"></script>
@endsection
