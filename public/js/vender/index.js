...
(function ($) {
    $(document).ready(function () {
        ...
        if (sessionStorage.hasOwnProperty('tab_vender')
            && sessionStorage.tab_vender != 'undefined'
            && $(`ul#tab-status li a[data-type="${sessionStorage.tab_vender}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_vender}"]`).tab('show')
        } else {
            // first tab
            $("ul#tab-status li:first-child a").tab('show')
        }
        // save active tab to session storage
        sessionStorage.setItem('tab_vender', $("ul#tab-status li a.active").attr('data-type'))
        ...
        $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
            // save active tab to session storage
            sessionStorage.setItem('tab_vender', $(event.target).attr('data-type'))
