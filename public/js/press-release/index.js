...
(function ($) {
    ...
    if (sessionStorage.hasOwnProperty('tab_press_release')
        && sessionStorage.tab_press_release != 'undefined'
        && $(`ul#tab-status li a[data-type="${sessionStorage.tab_press_release}"]`).length > 0) {
            $(`ul#tab-status li a[data-type="${sessionStorage.tab_press_release}"]`).tab('show')
    } else {
        // first tab
        $("ul#tab-status li:first-child a").tab('show')
    }
    // save active tab to session storage
    sessionStorage.setItem('tab_press_release',$("ul#tab-status li a.active").attr('data-type'))
    ...
    $('ul#tab-status li a[data-toggle="tab"]').on('shown.bs.tab', function (event) {
        // save active tab to session storage
        sessionStorage.setItem('tab_press_release',$(event.target).attr('data-type'))
