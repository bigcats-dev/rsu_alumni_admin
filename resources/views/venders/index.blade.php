    ...
    <div class="modal fade" id="modalDisApprove" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <form action="{{route("vender.approve",["vender" => ":id"])}}" id="frmDisApprove" method="POST">
                    @csrf
                    <input type="hidden" name="action" value="2">
                    <div class="modal-header">
                        <h4 class="modal-title"><strong>ไม่อนุมัติ Vender <i class="far fa-times-circle text-danger"></i> </strong></h4>
...
@section("script")
    <script>
        var configTable = Object.assign($.fn.dataTable.defaults,{
            ...
            order:[[ 0, "asc" ]],
            ajax: {
                url: '{{ route("vender.index") }}',
                type: "GET",
                data: function(data) {
                    data.type = sessionStorage.hasOwnProperty('tab_vender')
                            ? sessionStorage.tab_vender
            ...
            columns:[
                ...
                {data: 'corporation_name', orderable: false, width: '10%'},
                {data: 'corporation_no', orderable: false, width: '10%'},
                {data: 'email', orderable: false, width: '10%'},
                {data: 'tel_1', orderable: false, width: '10%'},
                {
                    data: 'approved',
                    orderable: false,
            ...
            rowCallback: function(row, data) {
                const {vender_id,action} = data
                $(row).find('[data-toggle="tooltip"]').tooltip()
                // active
                $(row).find(`input[id="active_${vender_id}"]`)
                    .on('click',function(e){
                        $.post(action.active,function(){},'json')
                    })
                // approve
                $(row).find(`form[id="frmApprove_${vender_id}"] button[type="button"]`)
                ...
                // delete
                $(row).find(`form[id="frmDestroy_${vender_id}"] button[type="button"]`)
                    .on('click',async function(e){
                        e.preventDefault()
                        const rs = await confirmAlert.fire({
                ...
                // restore
                $(row).find(`button[id="btn-restore_${vender_id}"]`)
                    .on('click',async function(e){
                        e.preventDefault()
                        const rs = await confirmAlert.fire()
                        if(rs) {
    ...
    <script src="{{ URL::asset("js/vender/index.js?t=".time()) }}"></script>
@endsection